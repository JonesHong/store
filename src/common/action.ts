...
export abstract class Action {
  ...
  private _actionId = `action-${uuidv4()}`;
  ...
  setParent(parent: Action) {
    ...
    if (parent.actionId == this._parentId) {
      ...
      const _logger = Logger.error(this.type, 'setParent with wrong parent.', {
  ...
  addChild(child: Action) {
    ...
    if (child._parentId == this.actionId) {
      ...
      const _logger = Logger.error(this.type, 'addChild with wrong child.', {
        isPrint: Main.printMode !== 'none',
      });
      if (envType == 'browser' && _logger['options']['isPrint'])
        console.error(_logger['_str']);
...
export const transferActionMapToActionList = function (
  actionMap: object
): string[] {
...
export const replaceDefaultToEntity = (
  defaultName: string,
  reducerName: string
) => {
  ...
  const reducerNameReg = upperCaseFirst(reducerName);
  const replaceReg = /Default/;
...
export const transferDefaultToEntity = (reducerName: string) => {
  ...
  const actionMap = _.cloneDeep(CommonActionMap);
  // await Promise.all(
  Object.entries(actionMap).map((actionEntry) => {
    const _key = actionEntry[0],
      _val = actionEntry[1];
    actionMap[_key] = replaceDefaultToEntity(_val, reducerName);
  });
  // );

  const _payload: { actionMap: typeof CommonActionMap; methodMap: MethodMap } =
    {
      actionMap,
      methodMap: {
        ...
        UpdateMany: (reducerName, entities) =>
          new UpdateMany(reducerName, entities),
        UpsertOne: (reducerName, entity) => new UpsertOne(reducerName, entity),
        UpsertMany: (reducerName, entities) =>
          new UpsertMany(reducerName, entities),
        CompareSettlement: (reducerName, settlement) =>
          new CompareSettlement(reducerName, settlement),
      },
    };

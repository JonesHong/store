...
class _Logger {
    ...
    private constructor() {
        if (envType == "nodejs") {
            ...
            this._logPath = path.resolve(`${_logFolderPath}/${DateString("yyyy.LL.dd.HH.mm.ss")}.log`);
            fs.writeFileSync(this._logPath, `Log file open, ${DateString()}`, { encoding: "utf-8" });
        }
    }
    ...
    private _baseHandler({ _str, options }: { _str: string, options: Options }) {
        // let _str, options;
        if (!!options) {
            const isOptionsProp = (prop: "isPrint" | "execTime" | "payload") => options.hasOwnProperty(prop);
    ...
    public log(type: LoggerType, message: string, options: Options = { isPrint: true }): void | { _str: string, options: Options } {
        let _str = `${chalk.green(`[Nrwl] ${this._processId} - `)}${DateString()}  ${chalk.yellow.bold(`[${type}]`)} ${chalk.green(message)}`;
    ...
    public error(type: LoggerType, message: string, options: Options = { isPrint: true }): void | { _str: string, options: Options } {
        let _str = `${chalk.green(`[Nrwl] ${this._processId} - `)}${DateString()}  ${error(`[${type}] ${message}`)}`;
        ({ _str, options } = this._baseHandler({ _str, options }));
        if (envType == "browser") {
            return { _str, options }
        }
    }
    public warn(type: LoggerType, message: string, options: Options = { isPrint: true }): void | { _str: string, options: Options } {
        let _str = `${chalk.green(`[Nrwl] ${this._processId} - `)}${DateString()}  ${warn(`[${type}] ${message}`)}`;
        ({ _str, options } = this._baseHandler({ _str, options }));
        if (envType == "browser") {
            return { _str, options }
        }

import { catchError, concatMap, delay, filter, last, map, mergeMap, reduce, tap, toArray } from "rxjs/operators";
import { from, Observable, of, pipe, throwError, UnaryFunction } from "rxjs";
import { IEntityRelationConfig, RelationDescription, RelationshipByType } from "./interface/relation.interface"
import { settlement, Store } from "./store";
import * as _ from "lodash";
import { Reducer } from "./reducer";
import { Entity } from "./entity";
import { addMany, cloneAndReset, removeMany, removeOne, updateMany } from "./adapter";
...
export class Relation {
        ...
        from(entitiesConfig)
            .pipe(
                mergeMap(entityConfig => {
                    ...
                    return from(relationDescriptions)
                        .pipe(
                            map(relationDescription => {
                                let fromEntity = `${entityName[0].toUpperCase()}${entityName.slice(1)}`,
    ...
    fromJDL(RelationshipByType: RelationshipByType) {
        let relationTypeEntities: [string, Set<string>][] = Object.entries(RelationshipByType);
            ...
            let splitString = str.split(" to "),
                ...
                isBracketLegal = (() => {
                    ...
                    splitString.map(halfStr => {
                        if (!!payload) {
                            payload = (curlyBracketsTest(halfStr) && parenthesesTest(halfStr)) ||
        ...
        from(relationTypeEntities)
            .pipe(
                mergeMap(relationTypeEntity => {
                    ...
                    return from(relationSet)
                        .pipe(
                            map(_sourceRelationConfig => {
                                let { fromEntity, fromRelationshipName, fromReferenceDisplay,
                                    toEntity, toRelationshipName, toReferenceDisplay } = regexpMatcher(_sourceRelationConfig);
                                let payload: any = {
                                ...
                                let renameFromEntity = `${fromEntity[0].toLowerCase()}${fromEntity.slice(1)}`;
     ...
     * @returns 
     */
    public getStoreWithRelation_v1<initialState, Reducers>(store: Store<initialState, Reducers>, options: { config: IEntityRelationConfig }): Observable<any> {
        let storeClone;
        return store.state$.pipe(
            ...
            mergeMap(state => {
                ...
                return from(Object.entries(storeClone.state))
                    .pipe(
                        reduce((totalState, nextStateEntry: [string, Reducer<any, any>]) => {
                            ...
                            if (!!sourceRelationConfigs) {
                                ...
                                sourceRelationConfigs.map(async (_sourceRelationConfig) => {
                                    let targetEntity = `${_sourceRelationConfig['targetEntity'][0].toLowerCase()}${_sourceRelationConfig['targetEntity'].slice(1)}`,
                                        sourceReducer = reducers[sourceEntity],
                                        targetReducer = reducers[targetEntity];

                                    let targetReducerRelationConfigs = options['config'][targetEntity].find(config => config['targetEntity'] == `${sourceEntity[0].toUpperCase()}${sourceEntity.slice(1)}`);
                                    let regexForMany = new RegExp("ManyTo");
                                    let isMatchMany = regexForMany.test(_sourceRelationConfig['type']);
                                    ...
                                    if (!targetReducerRelationConfigs) {
                                        ...
                                        let sourceStateEntries: [string, Entity][] = Object.entries(sourceState['entities'])
                                        // await Promise.all(
                                        sourceStateEntries.map(entity => {
                                            ...
                                            switch (_sourceRelationConfig['type']) {
                                                case "OneToOne": {
                                                    targetEntities = targetReducer.selectRelevanceEntity({ key: `${_sourceRelationConfig['relationshipName']}Id`, value: entityId });
                                                    ...
                                                    targetEntities.length !== 0 ? entityValue.setOneToOne(targetEntities) : null;
                                                    break;
                                                }
                                                case "OneToMany": {
                                                    // target是多的那邊
                                                    targetEntities = targetReducer.selectRelevanceEntity({ key: `${targetReducerRelationConfigs['referencesField']}`, value: entityId });

                                                    targetEntities.length !== 0 ? entityValue.addManyToOne(targetEntities, { target: targetReducerRelationConfigs, source: _sourceRelationConfig }) : null;
                                                    // targetEntities.length !== 0 ? entityValue.addManyToOne(targetEntities) : null;
                                                    break;
                                                }
                                                case "ManyToOne": {
                                                    // 依照
                                                    targetEntities = targetReducer.selectRelevanceEntity({ key: `${targetReducerRelationConfigs['referencesEntity']}`, value: entityValue[targetReducerRelationConfigs['referencesEntity']] });
                                                    ...
                                                    targetEntities.length !== 0 ? entityValue.setOneToMany(targetEntities, { target: targetReducerRelationConfigs, source: _sourceRelationConfig }) : null;
                                                    // targetEntities.length !== 0 ? entityValue.setOneToMany(targetEntities) : null;
                                                    break;
                                                }
                                                case "ManyToMany": {
                                                    targetEntities = targetReducer.selectRelevanceEntity({ key: `${_sourceRelationConfig['referencesField']}IdList`, value: entityId });
                                                    ...
                                                    targetEntities.length !== 0 ? entityValue.addManyToMany(targetEntities) : null;
                                                    break;
                                                }
                                            }
    ...
    public StoreRelation<initialState, Reducers>(store: Store<initialState, Reducers>, config: IEntityRelationConfig) {
        let count = 0;
        ...
        let _reducer: Reducer<any, any>,
            ...
            _sourceEntity, _sourceState, _sourceRelationConfigs: RelationDescription[],
            _targetEntity, _targetState, _targetRelationConfigs: RelationDescription[], _targetRelationConfig: RelationDescription;
                    ...
                    if (!!_sourceRelationConfigs && _sourceRelationConfigs.length !== 0) {
                        return from(_sourceRelationConfigs).pipe(
                            map(_sourceRelationConfig => {
                                ...
                                _targetRelationConfig = _targetRelationConfigs.find(config => config['targetEntity'] == `${_sourceEntity[0].toUpperCase()}${_sourceEntity.slice(1)}`);

                                let relevanceEntities = [];
                                switch (_sourceRelationConfig['type']) {
                                    case "OneToOne": {
                                        ; relevanceEntities = selectRelevanceEntity(_targetState, { key: `${_sourceRelationConfig['relationshipName']}Id`, value: entityId });
                                        relevanceEntities.length !== 0 ? entity.setOneToOne(relevanceEntities, { target: _targetRelationConfig, source: _sourceRelationConfig }) : null;
                                        // relevanceEntities.length !== 0 ? entity.setOneToOne(relevanceEntities) : null;
                                        break;
                                    }
                                    case "OneToMany": {
                                        // target是多的那邊
                                        relevanceEntities = selectRelevanceEntity(_targetState, { key: `${_targetRelationConfig['referencesField']}`, value: entityId });

                                        // if (entityId == "constructionLocation-33941722-a859-43c5-920a-34015ca5d730") {
                                        // console.log(23823098, relevanceEntities)
                                        // }
                                        relevanceEntities.length !== 0 ? entity.addManyToOne(relevanceEntities, { target: _targetRelationConfig, source: _sourceRelationConfig }) : null;
                                        // relevanceEntities.length !== 0 ? entity.addManyToOne(relevanceEntities) : null;
                                        break;
                                    }
                                    case "ManyToOne": {
                                        // 依照
                                        relevanceEntities = selectRelevanceEntity(_targetState, { key: `${_targetRelationConfig['referencesEntity']}`, value: entity[_sourceRelationConfig['referencesField']] });
                                        relevanceEntities.length !== 0 ? entity.setOneToMany(relevanceEntities, { target: _targetRelationConfig, source: _sourceRelationConfig }) : null;
                                    ...
                                    case "ManyToMany": {
                                        ...
                                        relevanceEntities = selectRelevanceEntity(_targetState, { key: `${_sourceRelationConfig['referencesField']}`, value: entityId });
                                        relevanceEntities.length !== 0 ? entity.addManyToMany(relevanceEntities, { target: _targetRelationConfig, source: _sourceRelationConfig }) : null;
        ...
        return store.settlement$.pipe(
            filter((reducerSettlement: settlement) => {
                return !!store && (!this._reducerSettlement || this._reducerSettlement['_currentHash'] !== reducerSettlement['_currentHash'])
            }),
            // filter((reducerSettlement: settlement) => {
            ...
            mergeMap(reducerSettlement => {
                ...
                if (!!reducerSettlement['lastSettlement']['isChanged']) {
                    return of(reducerSettlement)
                        .pipe(
                            // update _stateClone
                            map(reducerSettlement => {
                                _sourceEntity = reducerSettlement['reducerName'];
                                _reducer = store['reducers'][reducerSettlement['reducerName']];
                                _sourceState = cloneAndReset(_stateClone[reducerSettlement['reducerName']]);
                                _values = {
                                    create: _reducer.createEntities(Object.values(reducerSettlement['lastSettlement']['create'])),
                                    update: _reducer.createEntities(Object.values(reducerSettlement['lastSettlement']['update'])),
                                    delete: Object.values(reducerSettlement['lastSettlement']['delete']),
                                };
                                _sourceRelationConfigs = config[reducerSettlement['reducerName']];
                                _values.create.length !== 0 ? _sourceState = addMany(_values.create, _sourceState) : null;
                                _values.update.length !== 0 ? _sourceState = updateMany(_values.update, _sourceState) : null;
                                // 先斷開連結才刪除
                                _values['delete'].map((id: string) => {
                                    let _entity: Entity = selectRelevanceEntity(_sourceState, { "key": 'id', "value": id })[0];
                            ...
                            mergeMap(reducerSettlement => {
                                if (_values['update'].length !== 0) {
                                    return from(_values['update']).pipe(
                                        _commonRelationPipe()
                                    )
                                }
                                else {

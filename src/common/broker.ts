...
export abstract class Broker {
    ...
    private _brokerInitial() {
        ...
        this._isReadyToDispatchSubscribe = this.isReadyToDispatch$.subscribe((isReady) => {
    ...
    getTopic(action: Action): BehaviorSubject<Action> | undefined {
        let type = this.getActionType(action);
    ...
    getTopicByActionType(actionType: string): BehaviorSubject<Action> | undefined {
        if (!this.topicMap.has(actionType)) {
            // console.error(`There is no topicName: ${actionType}`);
            let _logger = Logger.error(
                "getTopicByActionType",
                `There is no topicName: ${actionType}`
            );
            if (envType == "browser" && _logger['options']['isPrint']) console.error(_logger['_str']);
            return undefined;
        }
        return this.topicMap.get(actionType);
    }
    getTopicsByActionTypeList = (actionTypeList: string[]): BehaviorSubject<Action>[] => {
        let topicList$: BehaviorSubject<Action>[] = [];
        // await Promise.all(
        actionTypeList.map(actionType => {
            let topic$ = this.getTopicByActionType(actionType);
            !!topic$ ? topicList$.push() : null;
        })
        // )
        return topicList$;
    }
    ...
    addTopic(action: Action): BehaviorSubject<Action> {
        ...
        let type = this.getActionType(action);
        if (this._topicMap.has(type)) {
            // console.error(`Topic <${type}> is already exist.`);
            let _logger = Logger.error(
                "addTopic",
                `Topic <${type}> is already exist.`
            );
            if (envType == "browser" && _logger['options']['isPrint']) console.error(_logger['_str']);
            return topic$;
        }
        this._topicMap.set(type, topic$)
        return topic$
    }
    addTopicList = (actionList: Action[]): BehaviorSubject<Action>[] => {
        let topicList$: BehaviorSubject<Action>[] = [];
        if (!Array.isArray(actionList)) {
            // console.error("Input must be an Action array.");
            let _logger = Logger.error(
                "addTopicList",
                `Input must be an Action array.`
            );
            if (envType == "browser" && _logger['options']['isPrint']) console.error(_logger['_str']);
            return topicList$;
        }
        ...
        actionList.map(action => {
            let topic = this.addTopic(action);
            !!topic ? topicList$.push(topic) : null;
        })
        // )
        return topicList$
    ...
    addTopicsByActionTypeList = (actionTypeList: string[]): BehaviorSubject<Action>[] => {
        let topicList$: BehaviorSubject<Action>[] = [];
    ...
    addTopicObject = (actionMap: { [key: string]: any }): BehaviorSubject<Action>[] => {
        let topicList$: BehaviorSubject<Action>[] = [];
        // await Promise.all(
        Object.values(actionMap).map((value) => {
            // let _key = entry[0], _val = entry[1];
            topicList$.push(this.addTopic(value));
        })
        // )
        return topicList$;
    }
    ...
    dispatch(action: Action): void {
        ...
        if (!this.topicMap.has(type)) {
            ...
            let _logger = Logger.warn(
                "Broker",
                `This event didn't have a Subject.Please check Action config:`,
                { "payload": action }
            );
            if (envType == "browser" && _logger['options']['isPrint']) console.warn(_logger['_str']);
            // this.addTopic(action);
        } else {
            let theTopic = this.getTopic(action);

...
export abstract class Entity {
    ...
    setOneToOne = (theOne: Entity | Entity[], options?: OptionsRelationDescription | BuildRelationOptions) => {
        const method = (one: Entity) => {
            let _key = !!options && 'target' in options ?
                `_${options['target']['relationshipName']}` : // For options offer
                `_${one._name}`; // As default
            if (!!options && 'targetKey' in options && !this._relationKeysMap.has(_key)) this._relationKeysMap.set(_key, options['targetKey']);
            if (!!this[_key]) {
            ...
            if (!!options && 'source' in options) {
                this._relationKeysMap.set(_key, `_${options['source']['relationshipName']}`);
    ...
    setOneToMany = (theOne: Entity | Entity[], options?: OptionsRelationDescription | BuildRelationOptions) => {
        const method = (one: Entity) => {
            let _key = !!options && 'target' in options ?
                `_${options['target']['relationshipName']}` : // For options offer
                `_${one._name}`; // As default
            if (!!options && 'targetKey' in options && !this._relationKeysMap.has(_key))
            ...
            this[_key] = one;
            if (!!options && 'source' in options) {
                this._relationKeysMap.set(_key, this.relationKeyModify(`_${options['source']['relationshipName']}`));
    ...
    addManyToOne = (theMany: Entity | Entity[], options?: OptionsRelationDescription | BuildRelationOptions) => {
        // console.log("\naddManyToOne\n", theMany)
    ...
    addManyToMany = (theMany: Entity | Entity[], options?: OptionsRelationDescription | BuildRelationOptions) => {
        const method = (many: Entity) => {
            let _key = !!options && 'target' in options ?
                this.relationKeyModify(`_${options['target']['relationshipName']}`) : // For options offer
                `_${many._name}Map`; // As default
    ...
    breakAllReferences() {
        ...
        Array.from(this._relationKeysMap).map(relation => {
            let _key = relation[0], _targetKey = relation[1];

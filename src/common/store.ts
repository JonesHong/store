...
export class Store<initialState, Reducers> extends Broker {
  ...
  private _withRelation$: BehaviorSubject<initialState> = new BehaviorSubject(
    null
  );
  ...
  addReducer(reducer: Reducer<any, any>): void {
    const keywordToSlice = reducer?._name?.search(/Reducer/);
    ...
    if (!reducer?.listen) {
      const _logger = Logger.error('Store', `The reducer need to be an BLoC.`, {
        isPrint: Main.printMode !== 'none',
      });
      if (envType == 'browser' && _logger['options']['isPrint'])
        console.error(_logger['_str']);
    ...
    if (this.subscriptionMap.has(reducerName)) {
      const _logger = Logger.warn('Store', `Reducer already exist.`, {
        isPrint: Main.printMode !== 'none',
      });
      if (envType == 'browser' && _logger['options']['isPrint'])
    ...
    const reducer$ = reducer.listen((state) => {
  ...
  private buildRelationStore = () => {
    ...
    let RelationshipConfigTable: RelationshipConfigTable,
      ...
      LastSettlementToEntity: { create: Entity[]; update: Entity[] } = {
        create: [],
        update: [],
      };
    this.settlement$
      .pipe(
        ...
        filter(
          (settlement) =>
            !!settlement && !!StateClone && !!Relation.RelationshipConfigTable
        ),
        map((settlement) => {
          ...
          const { lastSettlement } = SettlementClone;
          const { reducerName } = SettlementClone;
          ...
          if (LastSettlementToValues['create'].length !== 0) {
            LastSettlementToEntity['create'] = theReducer.createEntities(
              LastSettlementToValues['create']
            );
            theState = addMany(LastSettlementToEntity['create'], theState);
          }
          if (LastSettlementToValues['update'].length !== 0) {
            Array.from(LastSettlementToValues['update']).map((entityData) => {
              const theEntity: Entity = theState['entities'][entityData['id']];
              // 斷開所有連結，稍後會重建
              ...
              const newEntity = theEntity.upsertData(entityData);
          ...
          if (LastSettlementToValues['delete'].length !== 0) {
            Array.from(LastSettlementToValues['delete']).map((id: string) => {
              const theEntity: Entity = theState['entities'][id];
              // 斷開所有連結
        ...
        mergeMap(({ SettlementClone, reducerName }) => {
          ...
          const { lastSettlement } = SettlementClone;
          const isCreateLengthBeZero = lastSettlement['create'].length == 0,
          ...
          const RelationBuilderObservable = (EntityList: Entity[]) => {
            ...
            // 遍歷所有的 Entity
            return from(EntityList).pipe(
              mergeMap((entity: Entity) => {
                if (
                  !theConfig ||
                  theConfig['_relationshipOptions'].length == 0
                ) {
                ...
                return from(theConfig['_relationshipOptions']).pipe(
                  map((relationshipOption) => {
                    ...
                    const findRelevanceAndBuildUp = (
                      ForeignKey: string,
                      ForeignKeyValue: string
                    ) => {
                      // let inputReducerState = StateClone[inputEntityName];
                      const relevanceEntities = selectRelevanceEntity(
                    ...
                    switch (relationshipOption.RelationType) {
                      ...
                      case 'ManyToOne': {
                        ...
                        // let inputReducerState = StateClone[inputEntityName];
                        const ForeignKey =
                            relationshipOption['inputEntityOptions'][
                              'displayField'
                            ],
                          ForeignKeyValue =
                            entity[
                              relationshipOption['thisEntityOptions'][
                                'displayField'
                              ]
                            ];
                      ...
                      case 'ManyToMany': {
                        ...
                        if (!ForeignKeyValues || ForeignKeyValues.length == 0)
                          break; // new! 考慮到 non-sql，忽略空值不綁關係 -20230715
                        for (const ForeignKeyValue of ForeignKeyValues) {
                          // 有兩種可能 string[] or Relationship[]
                          switch (typeof ForeignKeyValues) {
                            case 'string': {
                              // 是 string[]
                              findRelevanceAndBuildUp(
                                defaultRelationKey,
                                ForeignKeyValue
                              );
                              break;
                            ...
                            case 'object': {
                              ...
                              findRelevanceAndBuildUp(
                                defaultRelationKey,
                                ForeignKeyValue[defaultRelationKey]
                              );
                              break;
                            }
                            default: {
          ...
          const create$ = RelationBuilderObservable(
              LastSettlementToEntity['create']
            ),
            update$ = RelationBuilderObservable(
              LastSettlementToEntity['update']
            );
          return concat(
...
export const settlementToObject = () => {
  return pipe(
    map((settlement: Settlement) => {
      ...
      if (_create.length !== 0) {
        _payload.push(new AddMany(settlement.reducerName, _create).toObject());
      }
      if (_update.length !== 0) {
        _payload.push(new SetMany(settlement.reducerName, _update).toObject());
      }
      if (_delete.length !== 0) {
        _payload.push(
          new RemoveMany(settlement.reducerName, _delete).toObject()
        );
      }
      return _payload;
    })
